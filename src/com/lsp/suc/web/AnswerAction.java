 ...
 */
@Namespace("/suc")
@Results( { @Result(name = AnswerAction.RELOAD, location = "answer.action", type = "redirect") })
public class AnswerAction extends GeneralAction<AnswerInfo> {
	...
	private AnswerInfo entity=new AnswerInfo();
	private Long _id;
	...
	public String execute() throws Exception {
		...
		sortMap.put("sort", -1);
		fycount=baseDao.getCount(PubConstants.SUC_ANSWER, whereMap);
		
		if(Struts2Utils.getParameter("fypage")!=null){
			fypage=Integer.parseInt(Struts2Utils.getParameter("fypage"));
		}
		backMap.put("context", 0);
		List<DBObject> list=baseDao.getList(PubConstants.SUC_ANSWER,whereMap,fypage,10, sortMap,backMap);
		Struts2Utils.getRequest().setAttribute("AnswerInfoList", list);
	...
	public String delete() throws Exception {
		try {
			...
			baseDao.delete(PubConstants.SUC_ANSWER,_id);
			baseDao.delete(PubConstants.SUC_ANSREDETAIL,whereMap);
	...
	protected void prepareModel() throws Exception {
		if (_id != null) {
			//有custId查出来 用户信息
			entity = (AnswerInfo)UniObject.DBObjectToObject(baseDao.getMessage(PubConstants.SUC_ANSWER,_id),AnswerInfo.class);
		} else {
			entity = new AnswerInfo();
		}
	}
	...
	public String save() throws Exception {
		...
		try {
			if(_id == null){
				_id=mongoSequence.currval(PubConstants.SUC_ANSWER);	
			}
			entity.set_id(_id);
			entity.setToUser(SpringSecurityUtils.getCurrentUser().getToUser());
			entity.setCreatedate(new Date());
			baseDao.insert(PubConstants.SUC_ANSWER,entity);
