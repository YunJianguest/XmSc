...
<html>
	...
	<head>
	  ...
	  function ajaxjz(){//加载 
	    ...
	    issend=false;  
	    $.post('${ctx}/shop/miners!list.action?custid=${custid}&agid=${agid}&lscode=${lscode}&fypage='+fypage, submitData,
	       	function(json) { 
		    		...
		    		var v = json.list; 
		    		 for(var i=0;i<v.length;i++){  
		    		    xszf+='<li class="mui-table-view-cell mui-media miner-media">'
							...
							+'<div class="miner-txt-cont mui-col-xs-9">'+v.ptitle+''
						    +'<p class="mui-ellipsis"><span>运行周期:</span><span>'+v.time+'</span></p>'
							+'<p class="mui-ellipsis"><span>矿机编号:</span><span>'+v._id+'</span></p>'	
							+'</div>'	
							+'<div class="miner-buyBtn mui-col-xs-3">'
							+'<button type="button" class="mui-btn mui-btn-green miner-btnbuy">购买</button>'
							+'</div></div></li>';
					 }
		    		fypage++;
